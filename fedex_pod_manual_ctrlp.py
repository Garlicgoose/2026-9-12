...
5. 程序检测到查询结果页后，自动发送 Ctrl+P，并通过浏览器“Save to PDF/保存为 PDF”保存：运单号.pdf
6. 用户手动点击 FedEx 详情页/更多详情。
7. 程序检测到详情内容后，再用 Ctrl+P 保存为：运单号+.pdf
8. 完成后自动复制下一票运单号。
9. 页面异常、验证码、FedEx 服务错误、浏览器失联、PDF 保存失败时立即停止推进，
   不刷新、不重试 FedEx 请求；由用户在置顶面板点击“重试当前”或“跳过当前”。

依赖：
    py -m pip install openpyxl playwright pyautogui pywinauto

注意：
    Playwright 只用于连接你自己已经打开的 Chromium 浏览器并检测当前页面。
    真正的 PDF 保存通过 Ctrl+P + 浏览器打印预览完成。
    第一次使用前，请在浏览器打印预览里把目标设为 “Save to PDF / 保存为 PDF”；
    程序不会由自己打开 FedEx 网站或提交查询。
"""
...
APP_TITLE = "FedEx POD 半自动助手 - Ctrl+P打印版"
...
def _find_browser_window_for_page(page):
    """用 Windows UI Automation 找到当前 FedEx 所在的 Chrome/Edge 窗口。"""
    try:
        from pywinauto import Desktop
    except ImportError as exc:
        raise FedExManualError("缺少 pywinauto，请执行：py -m pip install pywinauto") from exc
    ...
    # 先按网页标题匹配，避免多开浏览器时选错窗口。
    if title:
        token = title[:50].casefold()
        for win in windows:
            try:
                text = (win.window_text() or "").casefold()
                if token and token in text and win.is_visible():
                    return win
            except Exception:
                pass

    # 再退化为查找可见的 Edge/Chrome 窗口。
    for win in windows:
        try:
            text = (win.window_text() or "").casefold()
            if win.is_visible() and ("microsoft edge" in text or "google chrome" in text):
                return win
        except Exception:
            pass
    raise FedExManualError("找不到当前 Edge/Chrome 窗口")


def _wait_for_print_button(timeout_seconds: float = 18.0):
    """等待 Chromium 打印预览中的 Save/保存按钮。"""
    try:
        from pywinauto import Desktop
    except ImportError as exc:
        raise FedExManualError("缺少 pywinauto，请执行：py -m pip install pywinauto") from exc
        ...
        for win in desktop.windows():
            try:
                ...
                for btn in win.descendants(control_type="Button"):
                    name = (btn.window_text() or "").strip().casefold()
                    if name in {"save", "保存"}:
                        return win, btn
                    if name in {"print", "打印"}:
                        saw_print_button = True
            except Exception:
                continue
        time.sleep(0.35)
    if saw_print_button:
        raise FedExManualError(
            "打印预览当前不是“Save to PDF/保存为 PDF”，而是普通打印机。"
            "请先手动把打印目标切换为 Save to PDF，再点“重试当前”。"
        )
    raise FedExManualError("Ctrl+P 后没有检测到浏览器打印预览的 Save/保存按钮")


def _wait_for_save_dialog(timeout_seconds: float = 10.0):
    try:
        from pywinauto import Desktop
    except ImportError as exc:
        raise FedExManualError("缺少 pywinauto，请执行：py -m pip install pywinauto") from exc

    pattern = re.compile(r"save as|另存为|save print output as|保存打印输出|保存为", re.I)
    deadline = time.monotonic() + timeout_seconds
    while time.monotonic() < deadline:
        desktop = Desktop(backend="uia")
        for win in desktop.windows():
            try:
                ...
                if win.is_visible() and pattern.search(title):
                    return win
            except Exception:
                continue
        time.sleep(0.25)
    raise FedExManualError("点击 Save 后没有检测到 Windows“另存为”窗口")


def _save_as_dialog_to_path(dialog, destination: Path) -> None:
    """在 Windows 标准另存为窗口中写入完整目标路径并保存。"""
    destination = destination.resolve()
    destination.parent.mkdir(parents=True, exist_ok=True)
    if destination.exists():
        # 本脚本只有缺失 PDF 才会进入打印，这里避免覆盖确认框干扰自动化。
        destination.unlink()

    edit = None
    try:
        candidate = dialog.child_window(auto_id="1001", control_type="Edit")
    ...
    if edit is None:
        try:
            ...
            if edits:
                # 标准文件对话框中通常最后一个 Edit 是“文件名”。
                edit = edits[-1]
        except Exception:
            pass
    if edit is None:
        raise FedExManualError("找不到“另存为”窗口中的文件名输入框")

    try:
        edit.set_focus()
        edit.set_edit_text(str(destination))
    except Exception as exc:
        raise FedExManualError(f"无法填写 PDF 保存路径：{exc}") from exc
    ...
    try:
        ...
        pass
    if save_btn is None:
        raise FedExManualError("找不到“另存为”窗口中的保存按钮")

    try:
        save_btn.click_input()
    except Exception as exc:
        raise FedExManualError(f"点击保存按钮失败：{exc}") from exc


def print_current_page(context, page, destination: Path) -> None:
    """真正调用浏览器 Ctrl+P，然后自动保存为 PDF。

    页面识别仍通过 CDP 完成；这里只负责打印当前已经加载好的网页，不刷新、不导航。
    """
    del context  # 与 CDP 打印版保持相同函数签名。
    if "fedex.com" not in str(page.url).casefold():
        raise FedExManualError("当前不是 FedEx 官网页面")
    try:
        import pyautogui
    except ImportError as exc:
        raise FedExManualError("缺少 pyautogui，请执行：py -m pip install pyautogui") from exc
    ...
    browser_window = _find_browser_window_for_page(page)
    try:
    ...
    try:
        ...
        preview_window, save_button = _wait_for_print_button()
        try:
            preview_window.set_focus()
        except Exception:
            pass
        save_button.click_input()
        dialog = _wait_for_save_dialog()
        _save_as_dialog_to_path(dialog, destination)
    except Exception:
        # 尽量关闭可能残留的打印预览；不做任何网页刷新。
        try:
            pyautogui.press("esc")
        except Exception:
            pass
    ...
    while time.monotonic() < deadline:
        if is_valid_pdf(destination):
            return
        time.sleep(0.25)
    raise FedExManualError(f"保存后未检测到有效 PDF：{destination.name}")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="FedEx POD 半自动助手 - Ctrl+P打印版")
